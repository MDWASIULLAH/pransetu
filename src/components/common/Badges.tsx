...
export const SeverityBadge = ({ severity }: { severity: SOSSeverity }) => {
  switch (severity) {
    case 'CRITICAL':
      return <span className="px-2 py-0.5 rounded-sm bg-red-900/50 text-red-400 border border-red-500/50 text-[10px] font-bold tracking-wider uppercase animate-pulse">Critical</span>;
    case 'HIGH':
    ...
    case 'LOW':
      return <span className="px-2 py-0.5 rounded-sm bg-blue-900/50 text-blue-400 border border-blue-500/50 text-[10px] font-bold tracking-wider uppercase">Low</span>;
  }
};
...
export const DeliveryPill = ({ state }: { state: DeliveryState }) => {
  const getStyle = () => {
    switch (state) {
      ...
      case 'RELAYED': return 'bg-blue-900/40 text-blue-400 border-blue-500/30';
      case 'RELAYING': return 'bg-yellow-900/40 text-yellow-400 border-yellow-500/30';
      case 'STORED': return 'bg-purple-900/40 text-purple-400 border-purple-500/30';
      case 'CREATED': return 'bg-gray-800 text-gray-400 border-gray-600';
      case 'CLOSED': return 'bg-gray-900 text-gray-500 border-gray-800 line-through';
    }
  };

  return (
    <span className={clsx("px-2 py-0.5 rounded-full border text-[9px] font-mono uppercase tracking-wide", getStyle())}>
      {state.replace('_', ' ')}
    </span>
  );
};
