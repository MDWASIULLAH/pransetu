    ...
    <div className="p-6 h-full flex flex-col max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-6 shrink-0">
        <div>
          ...
          <p className="text-on-surface-variant text-sm mt-1">Real-time incoming emergency signals from all channels.</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            <input 
              ...
              className="bg-surface-container border border-outline-variant/30 rounded-lg pl-9 pr-4 py-2 text-sm text-white focus:outline-none focus:border-blue-500 w-64"
            />
          </div>
          <button className="flex items-center gap-2 bg-[#1f2937] hover:bg-[#374151] border border-outline-variant/30 text-on-surface-variant px-4 py-2 rounded-lg text-sm transition-colors">
            <Filter size={16} /> Filter
          </button>
        </div>
      </div>

      <div className="flex-1 overflow-hidden bg-surface-container border border-outline-variant/30 rounded-xl flex flex-col">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-on-surface-variant">
            <thead className="bg-[#1f2937]/50 text-xs uppercase font-semibold text-on-surface-variant sticky top-0">
              <tr>
              ...
              {filteredList.map((sos) => {
                ...
                return (
                  <tr key={sos.id} className="hover:bg-[#1f2937]/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs font-sans">{sos.source}</td>
                    ...
                    <td className="px-6 py-4 font-sans">{sos.hopCount}</td>
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        <button 
                          onClick={() => setSelectedSOS(sos.id)}
                          className="px-3 py-1 bg-blue-900/30 hover:bg-blue-900/60 border border-outline-variant/30 text-on-surface rounded text-xs transition-colors"
                        >
                          INSPECT
                        </button>
                        {canAcknowledge && (
