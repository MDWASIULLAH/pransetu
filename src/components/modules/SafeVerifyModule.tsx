...
export const SafeVerifyModule = () => {
  ...
  const getStateStyles = (state: string) => {
    switch(state) {
      ...
      default: return { bg: 'bg-gray-800 text-gray-400 border-gray-700', icon: HelpCircle };
    }
  };
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Rule: No-answer is NEVER interpreted as SAFE. All data originates from active verification.</p>
        </div>
      </div>

      <div className="bg-[#111827] border border-[#1f2937] rounded-xl overflow-hidden">
        <table className="w-full text-left text-sm text-gray-400">
          <thead className="bg-[#1f2937]/50 text-xs uppercase font-semibold text-gray-300">
            <tr>
              ...
              safeVerifyRecords.map(record => {
                ...
                return (
                  <tr key={record.id} className="hover:bg-[#1f2937]/30">
                    <td className="px-6 py-4 font-mono text-white">{record.id}</td>
                    <td className="px-6 py-4 font-mono">{record.citizenPhone}</td>
                    <td className="px-6 py-4">{record.campaignId}</td>
                    <td className="px-6 py-4 font-mono text-xs">{record.callId}</td>
