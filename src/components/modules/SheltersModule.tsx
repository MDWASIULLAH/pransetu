    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Real-time capacity and resource monitoring for safe zones.</p>
        </div>
        <div className="text-right">
          <div className="text-3xl font-mono text-white font-bold">{stats.shelterOccupancy}%</div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {shelters.map(shelter => {
          ...
          return (
            <div key={shelter.id} className="glass-panel p-6 rounded-xl border border-[#1f2937] hover:border-gray-600 transition-colors">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-xl font-bold text-white flex items-center gap-2">
                    <Home size={20} className={isFull ? 'text-red-500' : 'text-blue-400'} /> 
                    {shelter.name}
                  </h3>
                  <div className="text-sm text-gray-400 mt-1">{shelter.district} District</div>
                </div>
                <div className={clsx(
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {shelter.facilities?.map(fac => (
                  <span key={fac} className="bg-gray-800 text-gray-300 border border-gray-700 px-2 py-1 rounded text-xs flex items-center gap-1">
                    <Box size={12} /> {fac}
                  </span>
                ))}
              </div>
