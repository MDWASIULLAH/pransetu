...
export const DisasterDominoEffect: React.FC<DisasterDominoEffectProps> = ({ onClose }) => {
  ...
  const getSeverityBadge = (sev: DominoNode['severity']) => {
    if (sev === 'CRITICAL') return 'bg-red-950 text-red-300 border-red-800';
    if (sev === 'WARNING') return 'bg-orange-950 text-orange-300 border-orange-800';
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-md p-3 sm:p-4 animate-in fade-in">
      <div className="bg-surface-container border border-outline-variant rounded-2xl w-full max-w-5xl shadow-2xl max-h-[92vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="p-4 sm:p-6 bg-surface-container-high border-b border-outline-variant flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-error/15 border border-error/40 flex items-center justify-center text-error">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="bg-error-container text-on-error-container text-[10px] font-mono font-bold px-2 py-0.5 rounded-full border border-error/30">
          ...
          <button
            onClick={onClose}
            className="text-on-surface-variant hover:text-on-surface p-2 rounded-xl hover:bg-surface-bright cursor-pointer"
          >
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-5">
          ...
          <div className="p-4 bg-surface-container-high rounded-xl border border-outline-variant/80">
            <div className="flex flex-wrap justify-between items-center gap-2 mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono font-bold text-error uppercase">PRIMARY DISASTER TRIGGER</span>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 pb-2 border-b border-outline-variant">
                <span className="w-6 h-6 rounded-lg bg-primary/20 text-primary flex items-center justify-center font-mono text-xs font-bold">1</span>
                <span className="text-xs font-mono font-bold text-on-surface uppercase">Infrastructure Failures</span>
              </div>
                ...
                <div
                  ...
                  className={`p-3 rounded-xl border transition-all cursor-pointer ${
                    selectedNodeId === node.id
                      ? 'bg-surface-bright border-primary ring-1 ring-primary/40 shadow-lg'
                      : 'bg-surface-container border-outline-variant hover:border-primary/50'
                  }`}
                >
                  <div className="flex justify-between items-start mb-1">
                    <span className="font-bold text-xs text-on-surface leading-snug">{node.title}</span>
                    <span className={`text-[9px] font-mono px-1.5 py-0.2 rounded border font-bold uppercase ${getSeverityBadge(node.severity)}`}>
                      {node.severity}
                    </span>
                  </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 pb-2 border-b border-outline-variant">
                <span className="w-6 h-6 rounded-lg bg-secondary/20 text-secondary flex items-center justify-center font-mono text-xs font-bold">2</span>
                <span className="text-xs font-mono font-bold text-on-surface uppercase">Lifeline Disruptions</span>
              </div>
              {DOMINO_TREE.filter((n) => n.category === 'CRITICAL_LIFELINE' || n.category === 'RELIEF_BOTTLENECK').map((node) => (
                <div
                  ...
                  className={`p-3 rounded-xl border transition-all cursor-pointer ${
                    selectedNodeId === node.id
                      ? 'bg-surface-bright border-secondary ring-1 ring-secondary/40 shadow-lg'
                ...
                >
                  ...
                  <div className="flex justify-between items-center mt-2 text-[11px] font-mono text-on-surface-variant">
                    <span>Impact: {node.impactPercentage}%</span>
                    <span className="text-secondary font-bold">Inspect →</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 pb-2 border-b border-outline-variant">
                <span className="w-6 h-6 rounded-lg bg-error/20 text-error flex items-center justify-center font-mono text-xs font-bold">3</span>
                <span className="text-xs font-mono font-bold text-on-surface uppercase">Human Health Vulnerabilities</span>
              </div>
              {DOMINO_TREE.filter((n) => n.category === 'HUMAN_HEALTH').map((node) => (
                <div
                  ...
                  className={`p-3 rounded-xl border transition-all cursor-pointer ${
                    selectedNodeId === node.id
                      ? 'bg-surface-bright border-error ring-1 ring-error/40 shadow-lg'
                      : 'bg-surface-container border-outline-variant hover:border-error/50'
                  }`}
                >
                  <div className="flex justify-between items-start mb-1">
                    <span className="font-bold text-xs text-on-surface leading-snug">{node.title}</span>
                  ...
                  <div className="flex justify-between items-center mt-2 text-[11px] font-mono text-on-surface-variant">
                    <span>Impact: {node.impactPercentage}%</span>
                    <span className="text-error font-bold">Inspect →</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="p-4 sm:p-5 bg-surface-container rounded-2xl border border-outline-variant space-y-3">
            <div className="flex flex-wrap justify-between items-start gap-2">
              ...
              <span className={`px-2.5 py-1 rounded-full text-xs font-mono font-bold uppercase border ${getSeverityBadge(selectedNode.severity)}`}>
            ...
            <div className="p-3 bg-green-950/40 border border-green-800/60 rounded-xl flex items-start gap-2.5">
              <span className="material-symbols-outlined text-status-green text-[20px] shrink-0 mt-0.5">shield</span>
              <div>
                <span className="text-xs font-mono font-bold text-status-green block uppercase">
        ...
        {/* Footer */}
        <div className="p-4 bg-surface-container-high border-t border-outline-variant flex justify-between items-center text-xs font-mono">
